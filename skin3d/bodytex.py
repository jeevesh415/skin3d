...
class BodyTexDataset:
    ...
    def scan_row(self, scan_id: str) -> pd.DataFrame:
        ...
        if len(scan_row) > 1:
            raise ValueError("Multiple rows returned. `scan_id` should be unique.")

        return scan_row

    def texture_filepath(self, scan_id: str, highres: bool = True) -> str:
        ...
        folder_filename = os.path.join(scan_row.scan_name.values[0], image_name)
    ...
    def annotation_filepath(
        ...
        if annotator is None:
            ...
            filepath = os.path.join(self.dir_multi_annotate, annotator, filepath)
    ...
    def annotation(
        ...
        ann_properties = lesion_properties_from_annotations(annotated_df, scan_id)
    ...
    def summary(self):
        ...
        print("Number of annotated validation scans: {}".format(len(valid_ids)))
        print("Number of annotated testing scans: {}".format(len(test_ids)))
        print("Number of annotated longitudinal scans: {}".format(len(long_ids)))
        print("Total number of lesions annotated: {}".format(len(annotations)))
        print(
            "Average annotated lesion width={:.2f}, height={:.2f}".format(
